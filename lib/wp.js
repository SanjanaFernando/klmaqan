const WP_BASE_URL =
  process.env.WP_API_BASE_URL || "https://klmaqan.w3icon.com/wp-json";
...
if (!WP_API_USERNAME || !WP_API_PASSWORD) {
  throw new Error(
    "Missing WP_API_USERNAME or WP_API_PASSWORD for the WordPress API",
  );
}

const WP_AUTHORIZATION = `Basic ${Buffer.from(
...
const freshRequest = {
  cache: "no-store",
  headers: {
    Authorization: WP_AUTHORIZATION,
